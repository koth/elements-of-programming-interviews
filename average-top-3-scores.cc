...
int get_top3_scores_sum(const multiset<int>& scores);

// @include
string find_student_with_top_3_average_scores(ifstream* ifs) {
  ...
  int current_top3_scores_sum = 0;
  for (const auto& scores : student_scores) {
    if (scores.second.size() >= 3) {
      int current_scores_sum = get_top3_scores_sum(scores.second);
      if (current_scores_sum > current_top3_scores_sum) {
        current_top3_scores_sum = current_scores_sum;
...
int main(int argc, char* argv[]) {
  ...
  string name = find_student_with_top_3_average_scores(&ifs);
