...
bool is_valid_number(const string& s) {
  ...
  for (size_t i = 0; i < trim_s.size(); ++i) {
    ...
    if (trim_s[i] == 'e' || trim_s[i] == 'E') {
      ...
      dot_idx = i;
    } else if (!(isdigit(trim_s[i]) || trim_s[i] == '+' || trim_s[i] == '-')) {
      return false;
    }
  }
  ...
  bool res = true;
  if (dot_idx != -1) {
    //cout << "1 " << std::boolalpha << res << endl;
    if (e_idx != -1) {
      res &= ((is_integer(trim_s, 0, dot_idx, true, false) && is_integer(trim_s, dot_idx + 1, e_idx, false, true)) || (is_integer(trim_s, 0, dot_idx, true, true) && is_integer(trim_s, dot_idx + 1, e_idx, false, false))) && is_integer(trim_s, e_idx + 1, trim_s.size(), true, false);
      //cout << "2 " << std::boolalpha << res << endl;
    } else {
      res &= (is_integer(trim_s, 0, dot_idx, true, true) && is_integer(trim_s, dot_idx + 1, trim_s.size(), false, false)) || (is_integer(trim_s, 0, dot_idx, true, false) && is_integer(trim_s, dot_idx + 1, trim_s.size(), false, true));
      //cout << "3 " << std::boolalpha << res << endl;
    }
  } else {
    if (e_idx != -1) {
      res &= is_integer(trim_s, 0, e_idx, true, false) && is_integer(trim_s, e_idx + 1, trim_s.size(), true, false);
      //cout << "4 " << std::boolalpha << res << endl;
    } else {
      res &= is_integer(trim_s, 0, trim_s.size(), true, false);
      // cout << "5 " << std::boolalpha << res << endl;
    }
  }
  return res;
}
