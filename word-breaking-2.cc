...
vector<string> word_breaking(const string& s,
  ...
  if (table.back()) {  // has answer.
    vector<string> res;
    generate_results(s, dict, 0, table, lengths, &res, &results);
  }
  return results;
}
...
void generate_results(const string& s,
                      ...
                      vector<string>* results) {
  if (idx == s.size()) {
    results->emplace_back(join_with_space(*res));
    return;
  }

  // Tries all possible combinations of strings in dict and stores the res
  // into results.
  for (const auto& x : lengths) {
    if (idx + x > s.size()) {
      break;
    ...
    if (table[idx + x] && dict.find(target) != dict.end()) {
      ...
      generate_results(s, dict, idx + x, table, lengths, res, results);
      res->pop_back();
    }
  }
}
