...
// @include
bool is_interleaving_string(const string& s1, const string& s2,
                            const string& s3) {
...
void small_test() {
  ...
  assert(!is_interleaving_string("aabcc", "dbbca", "aadbbbaccc"));
...
int main(int argc, char *argv[]) {
  ...
  if (argc == 4) {
    ...
    cout << s1 << " " << s2 << " " << s3 << endl;
    cout << boolalpha << is_interleaving_string(s1, s2, s3) << endl;
  } else {
    default_random_engine gen((random_device())());
