...
bool has_duplicate(const vector<vector<int>>& A, int start_row, int end_row,
...
bool is_valid_Sudoku(const vector<vector<int>>& A) {
  ...
  for (int i = 0; i < A.size(); ++i) {
    if (has_duplicate(A, i, i + 1, 0, A.size(), A.size())) {
      return false;
    }
  }
  ...
  for (int j = 0; j < A.size(); ++j) {
    if (has_duplicate(A, 0, A.size(), j, j + 1, A.size())) {
      return false;
    }
  }
