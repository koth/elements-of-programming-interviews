...
bool is_monochromatic(
  ...
  return pixel_sum == 0 || // totally white
    pixel_sum == // totally black
    (upper_right.i - lower_left.i + 1) * (upper_right.j - lower_left.j + 1);
}
...
shared_ptr<TreeNode> calculate_optimal_2D_tree_helper(
  ...
  const Point &lower_left, const Point &upper_right,
  unordered_map<Point,
    unordered_map<Point, shared_ptr<TreeNode>, HashPoint>, HashPoint> &table) {
  // Illegal retangle region, returns empty node
  if (lower_left > upper_right) {
    return shared_ptr<TreeNode>(new TreeNode{0, lower_left, upper_right});
  }

  if (table[lower_left].find(upper_right) == table[lower_left].end()) {
    if (is_monochromatic(image_sum, lower_left, upper_right)) {
      shared_ptr<TreeNode> ptr(new TreeNode{1, lower_left, upper_right});
      table[lower_left][upper_right] = ptr;
    } else {
      shared_ptr<TreeNode>
        ptr(new TreeNode{numeric_limits<int>::max(), lower_left, upper_right});
        ...
        for (int t = lower_left.j; t <= upper_right.j + 1; ++t) {
          if ((s != lower_left.i && s != upper_right.i + 1) ||
            ...
            vector<shared_ptr<TreeNode> > children = {
              ...
              calculate_optimal_2D_tree_helper(
                image, image_sum,
                Point{lower_left.i, t}, Point{s - 1, upper_right.j}, table),
              // NE rectangle
              calculate_optimal_2D_tree_helper(
              ...
              calculate_optimal_2D_tree_helper(
                image, image_sum,
                Point{s, lower_left.j}, Point{upper_right.i, t - 1}, table)};
            ...
            if (node_num < ptr->node_num) {
              ptr->node_num = node_num, ptr->children = children;
            }
          }
        }
      }
...
shared_ptr<TreeNode> calculate_optimal_2D_tree(
  ...
  for (int i = 0; i < image.size(); ++i) {
    partial_sum(image_sum[i].begin(), image_sum[i].end(), image_sum[i].begin());
