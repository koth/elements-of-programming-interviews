...
bool check_ans(const string &s) {
  unordered_map<char, int> hash;
  // Insert each char into hash
  for_each(s.begin(), s.end(), [&hash](const char &c) { ++hash[c]; });

  // A string can be permuted as palindrome if the number of odd time chars is less than or equal to 1
  int odd_count = 0;
  for (unordered_map<char, int>::iterator it = hash.begin(); it != hash.end() && odd_count <= 1; ++it) {
    if (it->second & 1) {
      ++odd_count;
    }
  }
