...
shared_ptr<BinaryTree<T> > reconstruct_post_in_orders_helper(
  ...
  if (post_e > post_s && in_e > in_s) {
    auto it = find(in.begin() + in_s, in.begin() + in_e, post[post_e - 1]);
...
template <typename T>
shared_ptr<BinaryTree<T> > reconstruct_post_in_orders(
  const vector<T> &post, const vector<T> &in) {
  return
    reconstruct_post_in_orders_helper(post, 0, post.size(), in, 0, in.size());
}
// @exclude
