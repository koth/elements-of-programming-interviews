...
// @include
int longest_subarray_with_distinct_entries(const vector<int>& A) {
  // Record the last occurrence of each entry.
  unordered_map<int, size_t> last_occurrence;
  size_t starting_idx = 0, ans = 0;
  for (size_t i = 0; i < A.size(); ++i) {
    auto res = last_occurrence.emplace(A[i], i);
    if (!res.second) {  // A[i] appeared before. Check its validity.
      if (res.first->second >= starting_idx) {
        ans = max(ans, i - starting_idx);
        starting_idx = res.first->second + 1;
      }
      res.first->second = i;
    }
  }
...
int main(int argc, char** argv) {
  ...
  for (int times = 0; times < 1000; ++times) {
    ...
    cout << endl;
    */
    int ans = longest_subarray_with_distinct_entries(A);
