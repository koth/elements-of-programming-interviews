...
int main(int argc, char *argv[]) {
  ...
  //    2   5
  //  1    4 6
  shared_ptr<BinaryTree<int> > non_symm_root = shared_ptr<BinaryTree<int> >(new BinaryTree<int>());
  non_symm_root->left = shared_ptr<BinaryTree<int> >(new BinaryTree<int>());
  non_symm_root->left->left = shared_ptr<BinaryTree<int> >(new BinaryTree<int>());
  non_symm_root->right = shared_ptr<BinaryTree<int> >(new BinaryTree<int>());
  non_symm_root->right->left = shared_ptr<BinaryTree<int> >(new BinaryTree<int>());
  non_symm_root->right->right = shared_ptr<BinaryTree<int> >(new BinaryTree<int>());
  assert(is_symmetric<int>(non_symm_root) == false);
  cout << boolalpha << is_symmetric<int>(non_symm_root) << endl;
  // symmetric tree test
  shared_ptr<BinaryTree<int> > symm_root = shared_ptr<BinaryTree<int> >(new BinaryTree<int>());
